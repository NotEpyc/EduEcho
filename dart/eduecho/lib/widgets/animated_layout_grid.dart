...
class _AnimatedLayoutGridState extends State<AnimatedLayoutGrid>
  ...
  void initState() {
    ...
    _controller = AnimationController(
      duration: const Duration(
        milliseconds: 600,
      ), // Match navbar animation duration
      vsync: this,
    );
  ...
  void _initializeAnimations() {
    ...
    _animations = List.generate(4, (index) {
      return Tween<Offset>(begin: Offset.zero, end: positions[index]).animate(
        CurvedAnimation(
          parent: _controller,
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < 4; i++) {
      ...
      final currentPos = Offset(
        (basePos.dx + (animOffset.dx * scaleX * 0.3)).clamp(
          0.0,
          size.width - (boxSize * scaleX),
        ),
        (basePos.dy + (animOffset.dy * scaleY * 0.3)).clamp(
          0.0,
          size.height - (boxSize * scaleY),
        ),
      );
